...
let LaptopCollection: {
	...
	isUsedOrNew: string | number | boolean,
	warrenty: number | string
} = {
	LaptapName: "dell",
	generation: "5th",
	core: "i6",
...
console.log(LaptopCollection);

// // NEATED OBJECT BY TYPE ALIAS
type MoblieForSell = {
	mobilename: string | number,
	model: number | string,
	price: number,
	warenty: number | string
	ram: string | number
	phonestorage : string |number
}
let moblieInfo : MoblieForSell = {
	mobilename: "vivo",
	model: "Y12A",
	price: 40000,
	warenty: "2years",
...
// length method
let fruit0ne = ["apple", "banana", "peach","orange"]
console.log(fruit0ne.length);

let fruitTwo = ["apple", "banana", "peach", "orange"];
for (let i = 0; i <= 3;i++){
console.log(fruitTwo[i]);
}
let fruitThree = ["apple", "banana", "peach", "orange", "strewberry", "kiwi"];
 fruitThree.push("began")
 fruitThree.push("moli")
 for (let i = 0; i <  fruitThree.length;i++){
	console.log( fruitThree[i]);
}
for (let phal of  fruitThree){  //for of loop only for array
console.log(phal);
}
//-----------------------------------------------
//enum
